...
- 情報隠蔽の重要性
  - 並行開発可能になり、スピードが上がる
  - デバッグや置き換えが単独で可能になり、保守の負荷が軽減される
  - 単独でプロファイリングと最適化が可能なので、効果的なパフォーマンスチューニングができる
  - コンポーネントの再利用を促進する
  - 大規模システムのリスクを軽減する
- トップレベルのクラスやインターフェース
  - パッケージプライベート, public
  - public は互換性をサポートする義務がある
  - ひとつのクラスからしか使われないクラス
    - ネストした private クラスにすることを検討する
- メンバー（フィールド, メソッド, ネストしたクラスやインターフェース）
  - private, パッケージプライベート, protected, public
  - public の API を設計した後は他のメンバーを private にする
  - private でも Serializable を実装していると漏れてしまう
  - public は永久にサポートしなければならない
  - protected メンバーの必要性は稀であるべき
  - オーバーライドするメソッドはアクセスレベルを低くできない（リスコフの置換原則）
  - テスト容易性のために、アクセス可能正を緩和するのはパッケージプライベートまでに留める
  - インスタンスフィールドは public にすべきではない
    - 値の制限を放棄
    - 一般的にスレッドセーフでない
    - 新たな内部データ表現に変更する柔軟性を放棄
    - static も同様だが、 public static final な定数は例外
      - 配列は常に可変であることに注意
        - Collections#unmodifiableList を使って不変なリストを公開する
        - 配列の場合はコピーを返す
- モジュールシステム
  - パッケージのグループ化
  - JDK 自身を除き、勧告的なもの
  - モジュールが広く使われるかは未知数
    - やむを得ない必要がない限りモジュールを避けるのが最善
- アクセススコープは可能な限り小さくする
- public のクラスは public のフィールドを持つべきではない

### 項目21 将来のためにインターフェース設計する

- Java8 でデフォルトメソッド構文が追加された
  - 既存のインターフェースへのメソッド追加が可能になった
- インターフェースへのメソッド追加はリスクがある
  - すべての普遍式を維持するデフォルトメソッドは必ずしも書けない
  - Apache Commons の SynchronizedCollection
    - オーバーライドされていないが、同期される約束を果たせない
- デフォルトメソッドの存在は実行時エラーの問題を理論的に起こしやすい
  - エラーや警告なしでコンパイルできてしまう
- 注意深いインターフェースの設計が重要
  - テストが大切
  - 最低限三つの異なる実装を行い、複数のクライアントプログラムを書く
- リリースされた後の修正に期待すべきではない

## 第5章 ジェネリックス

### 項目32 ジェネリックスと可変長引数を注意して組み合わせる

- ジェネリックスと可変長引数メソッドはうまく協調しない
  - ヒープ汚染の分かりづらい警告
  - 混在させると型システムを破れる
  - 不整合より利便性を優先させた結果
- ジェネリックの可変長パラメータに値を保存するのは安全ではない
- SafeVarargs アノテーション
  - メソッドの作成者が型安全であることを約束する
- 安全を保証するには
  - パラメータ配列に何も保存しない(上書きしない)
  - パラメータ配列（あるいは複製を）参照させない
- SafeVarargs はすべてのメソッドに付ける
  - 安全でないメソッドは作らない、ということ
- SafeVarargs はオーバライドできないメソッドにのみ付けられる
- 可変長パラメータを Listのパラメータに置き換えることもできる
  - メソッドが型安全なことをコンパイラが証明できる
  - 少し冗長で、少し遅い可能性がある
...
- ストリームが登場してもループは使われている
  - StreamはIterableを拡張していない
  - ストリームをシーケンス処理したり、シーケンスをストリーム処理したいならアダプタを実装する必要がある
- 妥当な理由がない限り、シーケンスを返すAPIは両方に対応するべき
  - Collection やそのサブタイプが最善の戻り値型
  - Iterable のサブタイプであり、streamメソッドを持つ
  - メモリには気をつける
- Collectionを使えないなら独自コレクションの実装を検討する
- もしStreamが将来Iterableを拡張するようになったら、ストリームを返すべき

### 項目48 ストリームを並列化するときは注意を払う

- 活性違反
  - liveness failuer
  - エラーが起きてもプログラム自体は終了しないエラー？
- 見境なくストリームパイプラインを並列化しない
  - limitを使っている場合、parallelにしてもパフォーマンスが向上する可能性は低い
- パフォーマンスの向上が見込めるもの
  - ArrayList, HashMap, HashSet, ConcurrentHashMap, 配列, int range, long range
    - 参照の局所性があるもの
    - サブレンジに低いコストで分割できる
      - spliterator
  - 終端操作がリダクション
...
- 安全性違反
  - safety violation
  - 不正な結果や予期できない振る舞いのエラー？
  - 仕様に従わないマッパー、フィルター、関数オブジェクト
  - 順序を保持するには終端操作をforEachOrderedにする
- 並列化から優れたパフォーマンス向上を得るには少なくとも10万の要素数が必要
  - 最適化に価値をもたせるなら変更前後でパフォーマンステストをする
- 適切な環境下ではコア数にほぼ比例したパフォーマンス向上になる
- 乱数のストリームを並列化するときは SplittableRandomを使う

## 第8章 メソッド

### 項目55 オプショナルを注意して返す

- 値を返さないかもしれないメソッドを書くための３つの方法
  - 例外
    - 例外的な状況でのみ使うべき
    - コストが高い
  - null
    - nullに対するコードを書く必要がある
    - 検査を行わなければ NullPointerException スローされる可能性
  - Optional
    - 概念的には値を返す
    - 空の結果を返せる
- Optional を返すメソッドで決して null を返さない
- Optional は値が返らない可能性に強制的に直面させる
- クライアントは処理を選択できる
  - デフォルト値を提供する
    - orElse
    - デフォルト値のコストが高いときは orElseGet
  - 選択された例外をスローする
    - orElseThrow
  - 戻り値があると分かっている
    - get
...
- オプショナルで包むべきでないもの
  ...
  - 配列
  - オプショナルを含むコンテナ型
- オプショナルはコストがかかるが、注意深い測定で決める
- ボクシングされた基本データ型のオプショナルは決して返さない
  - OptionalInt, OptionalLong, OptionalDouble を使う
- 戻り値以外でオプショナルを使うのは稀
  - キー、値、コレクションや配列の要素は、大抵オプショナルを使うことが不適切
  - インスタンスフィールドにあるのは bad smell
...
- 適切な値型があるなら、基本データ型であろうがオブジェクト参照であろうがそれを使う
  - なければ適切な値型を書くべき
- 基本データ型、enum、集合型が間違って文字列で扱われていることが多い
...
- 適切なインターフェースが存在するなら、すべてインターフェース型を使用して宣言すべき
  - keep your honest
  - 実装は切り替えやすくなり、プログラムが柔軟になる
    - 優れたパフォーマンス
    - 欠けていた望ましい機能の提供
- 適切なインターフェースがなければ、クラスで参照することは適切
  - 値型
    - String, BigInteger
  - クラスベースのフレームワークに属している場合
    - OutputStream, java.ioのクラス
  - インターフェースにない追加メソッドをクラスが提供している場合
    - Queue では定義されていないcomparator メソッドを持つPriorityQueue
- インターフェースがなければ、クラス階層中で必要な機能を提供しているもっとも上位のクラスを使用する

### 項目65 リフレクションよりインターフェースを選ぶ

- リフレクションの短所
  - 型検査の恩恵をすべて失う
  - コードがぎこちなく、冗長になる
  - パフォーマンスが悪くなる
- リフレクションを使う精巧なアプリケーション
  - コード解析ツール
  - 依存性注入フレームワーク
  - これらのツールも最近はリフレクションから離れてきている
- Java7で ReflectiveOperationException が導入された
- できるだけ、オブジェクトのインスタンス化のみの使用にとどめる
